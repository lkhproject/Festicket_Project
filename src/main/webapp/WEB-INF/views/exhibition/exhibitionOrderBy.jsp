...
<!-- 페스티벌 리스트 끝 -->
	...
	<div class="container" id="festPagingNum">
		<c:if test="${pageMaker.prev }">
			<a href="exhibitionOrderBy?pageNum=${pageMaker.startPage-5 }&orderOption=${param.orderOption}"><c:out value="${'<' }"></c:out></a>&nbsp;&nbsp;&nbsp;
		</c:if>
		...
		<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="num">
			<c:choose>
				...
				<c:otherwise>
					<a href="exhibitionOrderBy?pageNum=${num }&orderOption=${param.orderOption}">${num }</a>&nbsp;&nbsp;&nbsp;
				</c:otherwise>
			</c:choose>
		</c:forEach>
